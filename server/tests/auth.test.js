...
describe("AUTH FLOW", () => {
    ...
    it("POST /auth/register → 201", async () => {
        const res = await setup_js_1.api.post("/api/v1/auth/register").send(user);
    ...
    it("POST /auth/login → 200 + accessToken + refresh cookie", async () => {
        const res = await setup_js_1.api.post("/api/v1/auth/login").send({
            email: user.email,
    ...
    it("GET /auth/me → 200 (authorized)", async () => {
        const res = await setup_js_1.api
            .get("/api/v1/auth/me")
    ...
    it("POST /auth/refresh → 200 (cookie-based)", async () => {
        const res = await setup_js_1.api.post("/api/v1/auth/refresh");
        expect(res.status).toBe(200);
    ...
    it("POST /auth/logout → 204 + clear refresh cookie", async () => {
        const res = await setup_js_1.api.post("/api/v1/auth/logout");
